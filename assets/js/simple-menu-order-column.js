...
(function () {
	...
	/* global smoc_ui */
	// Ensure smoc_ui exists, fallback to default
	const smocConfig = window.smoc_ui ?? {};

	// Provide defaults for missing keys
	const config = {
		enableConfirm: smocConfig.enable_confirm ?? true,
		tabToNext: smocConfig.tab_to_next ?? true
	};
	...
	function smocInit() {
		...
		smocInputs = document.querySelectorAll('input[id^=smoc]');
		...
		smocInputs.forEach(smocInput => {
			...
			smocInput.addEventListener('focusout', () => {
				...
				if (smocInput.currentValue !== smocInput.value) {
					if (!config.enableConfirm || window.confirm(__('Should the menu order value be updated?', 'simple-menu-order-column'))) {
		...
		fetch(`${ajaxurl}?action=smoc_reorder&_wpnonce=${encodeURIComponent(nonce)}`, {
			...
			.then(response => {
				if (response.success) {
					...
					if (smocConfig.tab_to_next) {
						tabToNextInput(smocInput);
					}
				} else {
					smocInput.value = smocInput.defaultValue;
					showError();
	...
	function tabToNextInput(smocInput) {
		const inputs = Array.from(smocInputs);
		const pos = inputs.indexOf(smocInput) + 1;
